...
static const struct pwm_dt_spec pwm_ch1 = PWM_DT_SPEC_GET(DT_ALIAS(pwmch1));
//static const struct pwm_dt_spec pwm_ch1n = PWM_DT_SPEC_GET(DT_ALIAS(pwmch1n));
static const struct pwm_dt_spec pwm_ch2 = PWM_DT_SPEC_GET(DT_ALIAS(pwmch2));
#define SLEEP_MSEC	25U

void main(void)
{
	uint32_t pulse_width_ch1 = 152U;
	uint32_t pulse_width_ch2 = 1515U;
	int error = 0;
	...
	while (1) {
		error = pwm_set_pulse_dt(&pwm_ch1, pulse_width_ch1);
		if (error) {
			printk("Error ch1 %d: failed to set pulse width\n", error);
			return;
		}
		error = pwm_set_pulse_dt(&pwm_ch2, pulse_width_ch2);
		if (error) {
